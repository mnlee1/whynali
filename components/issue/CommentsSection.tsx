...
export default function CommentsSection({
    ...
    usePendingAction(
        'comment',
        (action) => issueId ? action.issueId === issueId : action.discussionTopicId === discussionTopicId,
        (action) => {
            if (action.parentId) {
            ...
            <div>
                ...
                <div className="flex items-center justify-between px-3 py-2 mb-3 bg-surface-muted border border-border rounded-xl">
                    <p className="text-xs text-content-secondary flex items-center gap-1.5">
                        ...
                        {safetyBotEnabled ? (
                            <span><span className="text-green-500 font-medium">세이프티봇</span>이 악성 댓글로부터 보호해요.</span>
                        ) : (
                            <span><span className="text-green-500 font-medium">세이프티봇</span>이 꺼져 있어요. 모든 댓글이 표시돼요.</span>
                        )}
                    </p>
                    <button
