/**
 * app/issue/[id]/error.tsx
 *
 * [이슈 상세 에러 페이지]
 *
 * 특정 이슈 페이지(/issue/[id])에서 데이터 로딩 실패 등의
...
export default function IssueError({ error, reset }: ErrorProps) {
    const router = useRouter()

    useEffect(() => {
        console.error('[Issue Error Boundary]', error)
    }, [error])
