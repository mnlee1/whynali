                            ...
                            <tr key={issue.id} className="hover:bg-surface-subtle">
                                <td className="px-4 py-3 w-24 whitespace-nowrap">
                                    <div className="flex flex-col gap-1">
                                        {issue.source_track === 'manual' && (
                                            <span className="inline-flex items-center justify-center w-10 h-6 text-xs rounded border font-medium bg-indigo-100 text-indigo-700 border-indigo-200">
                                                수동
                                            </span>
                                        )}
                                        {issue.is_merge_target && (
                                            <span className="inline-flex items-center justify-center w-10 h-6 text-xs rounded border font-medium bg-amber-100 text-amber-700 border-amber-200">
                                                병합
                                            </span>
                                        )}
                                    </div>
                                ...
                                <td className="px-2 py-3 text-sm w-28">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <div className="relative" ref={openDropdownId === issue.id ? dropdownRef : null}>
                                            <button
                                                onClick={() => setOpenDropdownId(openDropdownId === issue.id ? null : issue.id)}
                                                className="btn-neutral btn-sm text-xs px-1.5"
                                                title="더보기"
                                            >
                                                ···
                                            </button>
                                            {openDropdownId === issue.id && (
                                                <div className="absolute right-0 top-full mt-1 z-50 min-w-[88px] bg-surface border border-border rounded-lg shadow-lg py-1 text-xs">
                                                    {issue.approval_status === '대기' && (
                                                        <button
                                                            onClick={() => { handleApprove(issue.id); setOpenDropdownId(null) }}
                                                            className="w-full text-left px-3 py-1.5 hover:bg-surface-subtle text-green-700"
                                                        >
                                                            승인
                                                        </button>
                                                    )}
                                                    {(issue.approval_status === '대기' || issue.approval_status === '승인') && (
                                                        <button
                                                            onClick={() => { handleReject(issue.id); setOpenDropdownId(null) }}
                                                            className="w-full text-left px-3 py-1.5 hover:bg-surface-subtle text-orange-700"
                                                        >
                                                            반려
                                                        </button>
                                                    )}
                                                    {issue.approval_status === '반려' && (
                                                        <button
                                                            onClick={() => { handleRestore(issue.id); setOpenDropdownId(null) }}
                                                    ...
                                                    <button
                                                        onClick={() => { setMergeSourceIssue(issue); setOpenDropdownId(null) }}
                                                        className="w-full text-left px-3 py-1.5 hover:bg-surface-subtle text-purple-700"
                                                    >
                                                        병합
                                                    </button>
                                                    <div className="border-t border-border my-1" />
                                                    <button
                                                        onClick={() => { handleDelete(issue); setOpenDropdownId(null) }}
                                                        className="w-full text-left px-3 py-1.5 hover:bg-surface-subtle text-red-600"
                                                    >
                                                        삭제
                                                    </button>
                                                </div>
