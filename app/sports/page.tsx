...
export default async function SportsPage() {
    ...
    const [
        ...
        supabaseAdmin.from('issues').select('*', { count: 'exact' }).eq('approval_status', '승인').eq('visibility_status', 'visible').is('merged_into_id', null).eq('category', '스포츠').order('created_at', { ascending: false }).range(0, 19),
