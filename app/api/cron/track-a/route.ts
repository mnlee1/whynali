...
이슈 제목의 **핵심 주제 자체**를 직접 다룬 뉴스만 선택.
같은 카테고리·같은 인물이라도 다른 사건/주제면 반드시 제외.

판단 기준:
- "A가 B를 했다"는 이슈라면 → A와 B가 모두 포함된 뉴스만 O
- A만 나오거나 B만 나오는 뉴스는 X
- 같은 정치/연예 카테고리라도 주제가 다르면 X

예시:
- 이슈 "국힘 지지율 18% 최저치" → "국힘 지지율 18%" (O), "이재명 지지율 61%" (X), "트럼프 이란 협상" (X)
- 이슈 "장원영 우리은행 광고 공개" → "장원영 우리은행 CF" (O), "장원영 외모 화제" (X)
- 이슈 "WBC 한국 8강" → "한국 8강 확정" (O), "WBC 이탈리아 경기" (X)

불확실하면 반드시 제외. 관련 없는 뉴스를 포함하는 것이 아예 연결 안 하는 것보다 훨씬 나쁩니다.
...
이슈 제목의 핵심 사건과 직접 관련된 글만 선택.
같은 카테고리·같은 인물이라도 다른 사건이면 반드시 제외.
불확실하면 제외.
