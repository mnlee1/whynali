 ...
 * 핵심 인물/단체 키워드 (참고용)
 *
 * [2026-03-10] CORE_KEYWORDS 특혜 제거
 * - 그루핑 조건에서 제거됨 (모든 인물 동등 처리)
 * - mergeRelatedGroups에서만 참고용으로 사용
 * - 향후 완전 폐기 가능
 */
const CORE_KEYWORDS = [
    // 정치
 ...
 *
 * [2026-03-10 개선] 조건 완화 + genericWords 강화로 근본적 해결
 * - CORE_KEYWORDS 의존도 제거: 모든 인물을 동등하게 처리
 * - 조건 완화: 같은 카테고리 + 1개 키워드면 그루핑 (제니 케이스 해결)
 * - GROUPING_STOPWORDS 대폭 강화로 과도한 그루핑 방지
 *
 * 새 조건:
 * 1. 공통 키워드 2개 이상 (카테고리 무관)
 * 2. 공통 키워드 1개 + 같은 카테고리
 * 3. (폐지) CORE_KEYWORDS 특혜 제거
 *
...
export function groupItems(items: RawItem[]): CandidateGroup[] {
    ...
    for (const item of items) {
        ...
        for (const group of groups) {
            ...
            // [개선] 조건 완화: 2개 이상 또는 1개 + 같은 카테고리
            // CORE_KEYWORDS 의존도 제거로 모든 인물 동등 처리
            if (commonCount >= 2 || (commonCount >= 1 && sameCategory)) {
 ...
 *
 * [2026-03-10 개선] 조건 완화
 * - Jaccard 유사도 임계값: 0.4 → 0.3으로 하향
 * - 같은 카테고리 + 유사도 0.2 이상도 허용
 * - CORE_KEYWORDS는 참고용으로만 유지
 *
 * 그루핑 후 각 그룹의 대표 제목을 추출하여
 * Jaccard 유사도가 임계값 이상이면 같은 이슈로 병합.
 *
...
function mergeRelatedGroups(groups: CandidateGroup[]): CandidateGroup[] {
    ...
    for (let i = 0; i < groups.length; i++) {
        ...
        const baseCategory = baseGroup.items[0]?.category
        ...
        for (let j = i + 1; j < groups.length; j++) {
            ...
            const targetCategory = targetGroup.items[0]?.category
            ...
            const hasCommonCorePerson = baseTokens.some(t =>
                CORE_KEYWORDS.includes(t) && targetTokens.includes(t)
            )

            const sameCategory = !baseCategory || !targetCategory || baseCategory === targetCategory

            // [개선] 조건 완화
            // 1. 유사도 0.3 이상 (기존 0.4)
            // 2. 같은 카테고리 + 유사도 0.2 이상 (신규)
            // 3. 핵심 인물 공통 (기존 유지, 참고용)
            const shouldMerge =
                similarity >= 0.3 ||
                (similarity >= 0.2 && sameCategory) ||
                hasCommonCorePerson

            if (similarity >= 0.3 || hasCommonCorePerson || (similarity >= 0.2 && sameCategory)) {
                ...
                if (sameCategory) console.log(`  - 같은 카테고리: ${baseCategory}`)
            }
