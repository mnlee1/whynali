...
export async function generateAndCacheSummaries(
    ...
    // 커뮤니티 게시글 제목 조회 — 뉴스보다 구체적인 사건 내용을 담고 있어 발단 원인 추론에 활용
...
## [발단] 작성 특별 지침
발단 기사·최근 뉴스·온라인 참고 자료를 **함께** 참고해 논란의 실제 원인을 추론하세요.
- 사과문·해명·활동 중단 같은 결과/후속 내용은 발단 bullets에서 제외하세요
- 누가, 어디서(플랫폼/장소), 어떤 발언이나 행동이 문제가 됐는지 구체적으로 서술하세요
- 온라인 참고 자료는 미확인 정보일 수 있으므로 사실로 단정하지 말고 "~했다는 의혹이 제기됐다", "~한 것으로 알려졌다" 같이 헤징 표현을 사용하세요
...
## 출력 금지 규칙 (반드시 준수)
- 더쿠·네이트판·뽐뿌 등 출처 사이트명 절대 언급 금지
- "커뮤니티에서 공유되고 있다", "온라인에서 확산되고 있다", "커뮤니티에서 화제가 되고 있다" 등 커뮤니티 반응 자체를 bullet으로 쓰지 말 것
- 참고 자료의 제목·출처 정보를 그대로 복사하지 말 것 — 내용을 해석해 실제 사건만 작성
...
6. 각 bullet의 text에서 문장 전체를 통틀어 가장 중요한 사실을 최대 2곳까지만 골라 마크다운 \`**\`로 짧게 볼드 표시하세요 — 절이 여러 개인 문장이라도 절마다 볼드하지 말고, 전체에서 딱 1개 또는 2개만 고르세요. 고를 때는 숫자나 명사만 단독으로 끊지 말고 수식어를 포함한 명사구나 핵심 동사로 짧게 고르세요. 조사(은/는/이/가/을/를 등), 시제 표현("했"/"하고 있"), 종결어미("어요"/"습니다")는 반드시 볼드 밖에 일반체로 남기고, 절 전체나 문장 전체를 통째로 볼드하지 마세요
