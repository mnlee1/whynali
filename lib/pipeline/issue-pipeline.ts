...
⚠️ 연속 보도형 주제 판별: 아래 유형에 해당하면 날짜·수치·순위·퍼센트·"N일째"·"돌파"·"경신" 등 특정 시점 표현을 빼고 "[대상]+[카테고리]" 형태로 작성 (대상명 반드시 포함):
- 영화/드라마 흥행 성적, 시청률 / 스포츠 경기·시즌 성적 / 선거·투표 개표 현황 / 재난재해 진행 상황(태풍·산불·지진 등) / 시세(주가·환율·유가) 변동 / 감염병 확산 현황
- 그 외 향후 반복적으로 소식이 갱신될 것으로 예상되는 유형 (예: 재판·수사 진행 상황, 협상·외교 진행 상황, 서바이벌·오디션 순위 등)
예: "스파이더맨4 흥행 성적", "OO팀 시즌 성적", "OO 선거 개표 현황", "OO 태풍 피해 현황"
해당하지 않는 단발성 뉴스는 아래처럼 구체적으로:
...
주제 설명 (2~3줄, 60~100자): 뉴스 헤드라인처럼 절 하나를 먼저 던지고 "…"로 이어 핵심·반전을 드러내는 어조로 작성 (예: "10년째 조용히 만나던 연인과…결국 4월 24일 백년가약을 맺는다."). 사실관계는 정확하게, 어조만 극적으로.
⚠️ 이 정보의 출처가 커뮤니티/온라인 게시판이라는 사실이 절대 드러나면 안 됩니다.
"커뮤니티", "온라인 커뮤니티", "네티즌", "누리꾼", 특정 사이트명 등 출처를 암시하는
표현을 일절 쓰지 말고, 사건 자체의 사실관계만 서술하세요.
...
응답 형식 (JSON만):
{
  ...
  "topic": "옥택연 결혼",
  "topicDescription": "10년째 조용히 만나던 연인과…결국 4월 24일 백년가약을 맺는다."
}`
...
- 8~15자, 사실 중심
- 금지: "화제", "논란", "충격", "대박"
⚠️ 연속 보도형 주제(영화·드라마 흥행/시청률, 스포츠 시즌 성적, 선거 개표, 재난재해 진행상황, 시세 변동, 감염병 확산, 재판·수사·협상 진행상황, 서바이벌 순위 등)면 날짜·수치·순위·퍼센트·"N일째"·"돌파"·"경신" 등을 빼고 "[대상]+[카테고리]" 형태로 작성 (대상명 필수 포함). 그 외 단발성 뉴스는 구체적 표현 유지.
...
- threeLine: intro·bullets·conclusion 전체 내용을 종합해서 "상황 → 전개 → 현재 상태" 3줄로 다시 압축. 세 줄이 서로 겹치는 내용을 반복하지 않도록 각 줄에 다른 정보를 담을 것 (예: 첫 줄에 쓴 내용을 둘째 줄에서 다시 말하지 않기)

## 문체
pointSummary, 단계별 summary, brief(intro/bullets/conclusion/threeLine) 전부 예외 없이 "~했어요", "~하고 있어요", "~됐어요"처럼 친근한 해요체로 작성하세요. "~했다", "~였다", "~한다"로 끝나는 신문체나 "~습니다", "~입니다", "~합니다"로 끝나는 하십시오체, "~야", "~해" 같은 반말은 절대 쓰지 마세요
